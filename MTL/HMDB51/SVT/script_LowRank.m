...
K = D(idx_HMDB51,idx_HMDB51);

[U,S,V]=svd(K);
N_DDA = 200;
DDA = U(:,1:N_DDA);
S_DDA = S(1:N_DDA,1:N_DDA);
V_DDA = V(:,1:N_DDA);
K_rec = DDA*S_DDA*V_DDA';

%% Learn Regression
lambda = 0.5;
DDA_tr = DDA(Para.tr_sample_ind,:);
Mapping = (DDA_tr'*DDA_tr+lambda*eye(size(DDA_tr,2)))\(DDA_tr'*Z');

DDA_te = DDA(Para.ts_sample_ind,:);
Z_hat = DDA_te*Mapping;

Y = [DDA [Z';Z_hat] ; [zeros(size(Prototype,1),size(DDA,2)) Prototype]];
Omega = [ones(size(K_tr,1),size(DDA,2)+size(Z,1)) ; [ones(size(K_ts,1),size(DDA,2)) ones(size(K_ts,1),size(Z,1))] ; ...
    [zeros(size(Prototype,1),size(DDA,2)) ones(size(Prototype))]];
mask = Omega;
close all;
figure; imagesc(mask);
...
S_ts = U(Para.selected_ts_idx,:);
Proto_ts = U(end-24:end,:);

ts_ClassNo = Para.ClassNoPerVideo(Para.ts_sample_ind);

predict_ClassNo = knnsearch(Proto_ts,S_ts,'Distance','cosine');
