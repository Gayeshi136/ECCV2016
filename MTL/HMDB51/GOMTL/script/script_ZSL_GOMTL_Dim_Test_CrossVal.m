...
Latent_range = [10 15 20 22 25 30];
...
model_path = '/import/geb-experiments-archive/Alex/MTL/HMDB51/GOMTL/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL/HMDB51/GOMTL/Perf/';
...
for lambdaS = lambdaS_range
    for lambdaA = lambdaA_range
        for latent = Latent_range
            ...
            LatentPerf = [LatentPerf;{Para.lambdaS Para.lambdaA Para.Latent meanAcc}];
            
            [meanAcc,Acc] = func_ts_ZSL_GOMTL_Dim(Para,Model);
            
            DistributedPerf = [DistributedPerf;{Para.lambdaS Para.lambdaA Para.Latent meanAcc}];
