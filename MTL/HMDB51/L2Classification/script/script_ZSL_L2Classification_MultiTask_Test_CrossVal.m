...
Para.lambdaA0 = 1e-2;
Para.gammaA0 = 1e-3;
A0_model_path = '/import/geb-experiments-archive/Alex/MTL/HMDB51/L2Classification/Model/';


%% Run 50 random splits

model_path = '/import/geb-experiments-archive/Alex/MTL/HMDB51/L2ClassificationMTL/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL/HMDB51/L2ClassificationMTL/Perf/';
...
for lambda = lambda_range
    for gamma = gamma_range
        for latent = Latent_range
            ...
            model_filepath = sprintf([model_path,'L2ClassificationMTL_trial-%d_embed-%s_lambda-%g_gamma-%g_lat-%d_aug-0.mat'],trial,EmbeddingMethod,Para.lambda,Para.gamma,Para.latent);
            ...
            %% Load A0
%             A0_model_filepath = sprintf([A0_model_path,'L2Classification_trial-%d_embed-%s_lambda-%g_gamma-%g_task-dim_aug-0.mat'],trial,EmbeddingMethod,Para.lambdaA0,Para.gammaA0);
%             temp = load(A0_model_filepath);
%             
%             A0 = temp.Model.A;
% %             
%             A_mtl = Model.S'*Model.A;
%             synModel.A = A_mtl;
            
%             [meanAcc,Accuracy] = func_ts_ZSL_KernelizedL2Classification(K_te,V_te,Y_GT,Para,temp.Model);
%             [meanAcc,Accuracy] = func_ts_ZSL_KernelizedL2Classification(K_te,V_te,Y_GT,Para,synModel);
            
            [meanAcc,Accuracy] = func_ts_ZSL_KernelizedL2Classification_MultiTask(K_te,V_te,Y_GT,Para,Model);
            
            ESZSLPerf = [ESZSLPerf;{Para.lambda Para.gamma Para.latent meanAcc}];
            fprintf('lambda=%g, gamma=%g, task=%g acc=%.1f\n',Para.lambda,Para.gamma,Para.latent,meanAcc*100);
        end
    end
end
