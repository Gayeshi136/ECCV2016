...
%                     continue;
                ...
                %% Load Weight
                XYmodel_filepath = sprintf([weight_model_path,'LinearRegress_trial-%d_embed-%s_sigmaX-%g_sigmaY-%g_aug-1.mat'],trial,EmbeddingMethod,Para.sigmaX,Para.sigmaY);
                load(XYmodel_filepath,'Model');
                Z = Z*sparse(1:N_L,1:N_L,sqrt(model.weight));
                K = K*sparse(1:N_L,1:N_L,sqrt(model.weight));
                %% Train GOMTL Model
