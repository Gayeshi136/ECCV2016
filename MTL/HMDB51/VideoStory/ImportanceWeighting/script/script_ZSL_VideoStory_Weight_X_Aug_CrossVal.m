...
LatentDim_range = [50 100 150 200];
alpha = 0.2;
Para.SelfTraining = 0;
...
model_path = '/import/geb-experiments-archive/Alex/MTL_Weight/HMDB51/VideoStory_KLIEP/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL_Weight/HMDB51/VideoStory_KLIEP/Perf/';
if ~exist(perf_path,'dir')
    mkdir(perf_path);
end

weight_model_path = '/import/geb-experiments-archive/Alex/ImportanceWeighting/HMDB51/KLIEP_X/Model/';
    ...
    for lambdaA = lambdaA_range
        for lambdaS = lambdaS_range
            for LatentDim = LatentDim_range
                ...
                %% Load Weight
                Xmodel_filepath = sprintf([weight_model_path,'LinearRegress_trial-%d_embed-%s_sigma-%g_aug-1.mat'],trial,EmbeddingMethod,Para.sigmaX);
                load(Xmodel_filepath,'Model');
                Z = Z*sparse(1:N,1:N,sqrt(Model.weight));
                K = K*sparse(1:N,1:N,sqrt(Model.weight));
                %% Train GOMTL Model
