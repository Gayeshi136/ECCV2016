...
model_path = '/import/geb-experiments-archive/Alex/MTL/HMDB51/RMTL/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end
...
for trial = 1:50
    ...
    if ~exist(model_filepath,'file')
        ...
        Para.Z = [tr_LabelVec_HMDB51(Para.tr_sample_ind,:)];
        %% Train RMTL Model
        ...
        perf_filepath = sprintf([perf_path,'meanAcc_trial-%d_lambda0-%g_lambdat-%g_latent-0.mat'],trial,Para.lambda0,Para.lambdat);
        if ~exist(perf_filepath,'file')
            ...
            [meanAcc(trial),Acc{trial}] = func_ts_ZSL_RMTL_Dim(Para,Model);
