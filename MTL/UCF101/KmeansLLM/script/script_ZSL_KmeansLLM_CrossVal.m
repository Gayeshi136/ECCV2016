...
model_path = '/import/geb-experiments-archive/Alex/MTL/UCF101/KmeansLLM/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL/UCF101/KmeansLLM/Perf/';
...
for NumLLM = NumLLM_range
    
    Para.NumLLM = NumLLM;    % number of local model
    
    %%% Check if model is computed
    model_filepath = sprintf([model_path,'LinearRegress_trial-%d_embed-%s_lambda-%g_center-%d_aug-0.mat'],trial,EmbeddingMethod,Para.lambda,Para.NumLLM);
    ...
    JointX = [K ; Z];
    [inst_idx,center] = kmeans(JointX',Para.NumLLM);
    
    for llm_i = 1:Para.NumLLM
        
        idx = inst_idx==llm_i;
        
        A{llm_i} = Z(:,idx)/(K(idx,idx)+Para.lambda*sum(idx)*eye(sum(idx)));
        
    end
    Model.A = A;
    Model.Center = center;
    Model.TrClusterIdx = inst_idx;
