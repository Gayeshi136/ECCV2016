...
model_path = '/import/geb-experiments-archive/Alex/UCF101/ITF/FV/Zeroshot/jointcodebook/Embedding/DatasetSplit_tr-0.5_ts-0.5/HOF|HOG|MBH/RMTL_Dim/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/UCF101/ITF/FV/Zeroshot/jointcodebook/Embedding/DatasetSplit_tr-0.5_ts-0.5/HOF|HOG|MBH/RMTL_Dim/Perf/';
...
for trial = 1:50
    ...
    model_filepath = sprintf([model_path,'LinearRegress_trial-%d_embed-%s_lambda0-%g_lambdat-%g_task-dim.mat'],trial,EmbeddingMethod,Para.lambda0,Para.lambdat);
    if ~exist(model_filepath,'file')
        ...
        %% Train RMTL Model
        Model = func_tr_ZSL_MultiTask_Dim(Para);
        ...
        if ~exist(perf_filepath,'file')
            ...
            %% Test RMTL Model
            [meanAcc(trial),Acc{trial}] = func_ts_ZSL_MultiTask_Dim(Para,Model);
