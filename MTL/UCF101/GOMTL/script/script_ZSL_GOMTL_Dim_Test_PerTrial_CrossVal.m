...
model_path = '/import/geb-experiments-archive/Alex/MTL/UCF101/GOMTL/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL/UCF101/GOMTL/Perf/';
...
for trial = 1:5
    ...
    perf_filepath = fullfile(perf_path,sprintf('bestPerf_trial-%d.mat',trial));
        ...
        for lambdaA = lambdaA_range
            for latent = Latent_range
                ...
                model_filepath = sprintf([model_path,...
                    'LinearRegress_trial-%d_embed-%s_lambdaS-%g_lambdaA-%g_Lat-%d_task-dim_aug-0_norm-zscore.mat'],trial,EmbeddingMethod,Para.lambdaS,Para.lambdaA,Para.Latent);
                ...
                %% Test GOMTL Model
                [meanAcc,Accuracy] = func_ts_ZSL_GOMTL_ACC_Distributed(Para,Model);
                ...
                [meanAcc,Accuracy] = func_ts_ZSL_GOMTL_ACC_Latent(Para,Model);
    ...
    [PerfDistributed.bestAccTrial,indx] = max(Perf_Dist(:,4));
    PerfDistributed.bestlambdaS = Perf_Dist(indx,1);
    PerfDistributed.bestlambdaA = Perf_Dist(indx,2);
    PerfDistributed.bestLatent = Perf_Dist(indx,3);
    
    [PerfLatent.bestAccTrial,indx] = max(Perf_Lat(:,4));
    PerfLatent.bestlambdaS = Perf_Lat(indx,1);
    PerfLatent.bestlambdaA = Perf_Lat(indx,2);
    PerfLatent.bestLatent = Perf_Lat(indx,3);
