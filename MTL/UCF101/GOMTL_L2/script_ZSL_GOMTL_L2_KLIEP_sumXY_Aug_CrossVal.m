...
LatentProportion_range = [0.02 0.05 0.08 0.075 0.085 0.0867 0.1 0.16 0.17 0.18 0.2];
LatentProportion_range = [0.44];
alpha = 0.2;
Para.SelfTraining = 0;
...
%%% Load Per instance weight
IW_Model = load('/import/geb-experiments-archive/Alex/ImportanceWeighting/UCF101/KLIEP_sumXY/Model/LinearRegress_trial-1_embed-add_sigmaX-0.25_sigmaY-0.25_aug-1.mat');
IW_Model = IW_Model.Model;

Para.lambdaS = 1e-7;
Para.lambdaA = 1e-3;
Para.LatentProportion = 0.2;    % proportion of latent tasks

model_path = '/import/geb-experiments-archive/Alex/MTL/UCF101/GOMTL_L2_KLIEP_XY/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL/UCF101/GOMTL_L2_KLIEP_XY/Perf/';
...
for lambdaS = lambdaS_range
    for lambdaA = lambdaA_range
        for prop = LatentProportion_range
            ...
            model_filepath = sprintf([model_path,'LinearRegress_trial-%d_embed-%s_lambdaS-%g_lambdaA-%g_LatProp-%g_task-dim_aug-1.mat'],trial,EmbeddingMethod,Para.lambdaS,Para.lambdaA,Para.LatentProportion);
            ...
            %% Re-Weight Instances
            Z = Z*sparse(1:N,1:N,sqrt(IW_Model.weight));
            K = K*sparse(1:N,1:N,sqrt(IW_Model.weight));

            %% Train GOMTL Model
            [Model.S , Model.A , Model.L]=func_KernelizedGOMTL_L2_ncg(K,Z,N_L,Para.lambdaS,Para.lambdaA);
%             [Model.S , Model.A , Model.L]=func_KernelizedGOMTL_admm_ncg(K,Z,N_L,Para.lambdaS,Para.lambdaA);
