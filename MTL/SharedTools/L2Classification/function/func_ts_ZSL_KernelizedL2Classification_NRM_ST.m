...
function [meanAcc,Accuracy] = func_ts_ZSL_KernelizedL2Classification_NRM_ST(K,V,Y,Para,Model)
...
%% Do NRM hubness correction
Y_hat = 1-(Y_hat-min(Y_hat(:)))./(max(Y_hat(:))-min(Y_hat(:)));
Y_hat_norm = Y_hat./repmat(sqrt(sum(Y_hat.^2,2)),1,size(Y_hat,2));
[~,predict_ClassNo] = min(Y_hat_norm,[],1);
