...
function [map,ap] = func_ts_ZSL_RMTL_MAP_Dim_L2(Para,Model)
...
S_ts = (repmat(A0,size(At_bar,1),1)+At_bar)*D(Para.aug_tr_idx,Para.selected_ts_idx);   %D(selected_ts_idx,aug_tr_idx) * A;
% S_ts = (S_ts');
% S_ts = zscore(S_ts');
...
% S_ts = (S_ts - repmat(mu,size(S_ts,1),1))./repmat(sigma,size(S_ts,1),1);
