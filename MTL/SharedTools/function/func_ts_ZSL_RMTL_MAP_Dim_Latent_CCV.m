...
function [map,ap] = func_ts_ZSL_RMTL_MAP_Dim_Latent_CCV(Para,Model)
...
S_ts = A_tilde*D_CCV(Para.aug_tr_idx,Para.selected_ts_idx);   %D(selected_ts_idx,aug_tr_idx) * A;
...
Prototype = func_L2Normalization(Prototype*Inv_Sparse');
...
if Para.SelfTraining
    ...
    for c_i = 1:length(Para.idx_TestingSet)
        ...
        ts_class = Para.idx_TestingSet(c_i);
        
        label_test = Para.ClassNoPerVideo(Para.ts_sample_ind,ts_class)==1;

        ap(c_i) = func_CalcMap(confs(c_i,:), label_test);
