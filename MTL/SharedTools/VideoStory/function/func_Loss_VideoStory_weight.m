...
function [Loss] = func_Loss_VideoStory_weight(D,A,S,S_weight,Z,K,W,Para)


Term_Z_DS_weight = (Z-D*S)*W;
Term_S_AK_weight = (S-A*K)*W;


Loss = 1/Para.N_K*sum(sum(Term_Z_DS_weight.*Term_Z_DS_weight)) + Para.lambdaS*sum(sum(S_weight.*S_weight)) +...
    Para.lambdaD*sum(sum(D.*D)) + 1/Para.N_K*sum(sum(Term_S_AK_weight.*Term_S_AK_weight)) + Para.lambdaA*sum(sum(A.*A));
