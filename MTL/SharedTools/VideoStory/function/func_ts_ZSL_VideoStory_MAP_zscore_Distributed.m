...
function [map,ap] = func_ts_ZSL_VideoStory_MAP_zscore_Distributed(Para,Model)
...
S_ts = D_map*A*D(aug_tr_idx,selected_ts_idx);   %D(selected_ts_idx,aug_tr_idx) * A;
...
% Prototype = zscore((pinv(D_map)*Prototype')');

[Prototype,mu,sigma] = zscore(func_L2Normalization(Para.phrasevec_mat));
Prototype = Prototype(Para.idx_TestingSet,:);
% Prototype = func_L2Normalization(Prototype')';
...
%     confs = 1-pdist2(Prototype,S_ts,'cosine');
