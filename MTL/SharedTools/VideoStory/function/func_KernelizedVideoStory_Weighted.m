...
function [D,A,S,Loss]=func_KernelizedVideoStory_Weighted(K,Z,L,weight,lambdaD,lambdaA,lambdaS,param)
...
itr=1;
Loss(itr) = func_Loss_VideoStory_weight(D,A,S,S_weight,Z,K,W,Para);
...
while true
    ...
    %% Fix S Update D
    D = Z_weight*S_weight'/(S_weight*S_weight'+lambdaD*N_K*eye(N_L));
    
    %% Fix S Update A
    A = S_weight*Term_pinvKKt;
    
    %% Fix D,A Update S
    S = (D'*D+(lambdaS*N_K+1)*eye(N_L))\(D'*Z+A*K);
    S_weight = S*W;
