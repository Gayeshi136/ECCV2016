...
function [meanAcc,Accuracy] = func_ts_ZSL_LowRank(Para,Model)
...
D_z = size(Para.phrasevec_mat,2);
D_k = numel(aug_tr_idx);
...
S_ts = pinv(X)*[D(selected_ts_idx,aug_tr_idx) zeros(numel(selected_ts_idx),D_z)]';
S_ts =S_ts';
% try S_ts = A*D(aug_tr_idx,selected_ts_idx);   %D(selected_ts_idx,aug_tr_idx) * A;
% catch
%     a=1;
% end
...
%%% Generate Prototypes
Prototype = [zeros(numel(Para.idx_TestingSet),D_k) func_L2Normalization(Para.phrasevec_mat(Para.idx_TestingSet,:))];
Prototype = (pinv(X)*Prototype')';
