...
temp = load(sprintf([labelvector_path,'ClassLabelPhraseDict_mth-%s.mat'],Para.EmbeddingMethod));
...
model_path = '/import/geb-experiments-archive/Alex/Joint/OlympicSports/VideoStory/KLIEPX/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/Joint/OlympicSports/VideoStory/KLIEPX/Perf/';
                ...
                for LatentDim = Para.LatentDim_range
                    for sigmaX = Para.sigma_range
                        ...
                        model_filepath = sprintf([model_path,...
                            'VideoStory_trial-%d_lambdaD-%g_lambdaA-%g_lambdaS-%g_LatDim-%d_sigX-%g_aug-1_KLEIPX_ccv_weight.mat'],...
                            trial,lambdaD,lambdaA,lambdaS,LatentDim,Para.sigmaX);
                        ...
                        %% Load Data Split
                        load(sprintf([zeroshot_datasplit_path,'DatasetSplit_tr-%.1f_ts-%.1f_t-%d.mat'],Para.perc_TrainingSet,Para.perc_TestingSet,trial));
                        ...
                        [AccLat,Acc] = func_ts_ZSL_VideoStory_MAP_zscore_Latent(Para,Model);
                        
                        %                     Perf_Lat = [Perf_Lat;[Para.lambdaD Para.lambdaA Para.lambdaS Para.LatentDim AccLat]];
                        %                     fprintf('trial=%d lambdaD=%g lambdaA=%g lambdaS=%g Lat=%d\n latent acc=%.2f\n\n',...
                        %                         trial,Para.lambdaD,Para.lambdaA,Para.lambdaS,Para.LatentDim,100*AccLat);
                        
                        Perf_Lat = [Perf_Lat;[Para.sigmaX Para.LatentDim AccLat]];
                        fprintf('trial=%d sigmaX=%g Lat=%d\n latent acc=%.2f\n\n',...
                            trial,Para.sigmaX,Para.LatentDim,100*AccLat);
                        %% Save Performance
...
[PerfLatent.bestAccTrial,indx] = max(Perf_Lat(:,3));
% PerfLatent.bestLambdaD = Perf_Lat(indx,1);
% PerfLatent.bestLambdaA = Perf_Lat(indx,2);
% PerfLatent.bestLambdaS = Perf_Lat(indx,3);
PerfLatent.bestLatentDim = Perf_Lat(indx,2);
PerfLatent.bestsigmaX = Perf_Lat(indx,1);
...
bestAccTrial_sigmaX(trial) = PerfLatent.bestsigmaY;
