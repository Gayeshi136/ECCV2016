...
model_path = '/import/geb-experiments-archive/Alex/Joint/OlympicSports/VideoStory/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/Joint/OlympicSports/VideoStory/Perf/';
if ~exist(perf_path,'dir')
    mkdir(perf_path);
end

%%% Importance Weighting
weight_model_path = '/import/geb-experiments-archive/Alex/ImportanceWeighting/OlympicSports/KLIEP_XY/Model/';
        ...
        for lambdaA = Para.lambdaA_range
            for lambdaS = Para.lambdaS_range
                for LatentDim = Para.LatentDim_range
                    ...
                    %% Load weight
                    XYmodel_filepath = sprintf([weight_model_path,'LinearRegress_trial-%d_embed-%s_sigmaX-%g_sigmaY-%g_aug-1_zscore.mat'],trial,Para.EmbeddingMethod,Para.sigmaX,Para.sigmaY);
