...
for trial = 1
            ...
            for lambdaS = Para.lambdaS_range
                for LatentDim = Para.LatentDim_range
                    for sigmaX = Para.sigma_range
                        ...
                        Perf_Lat = [Perf_Lat;[Para.lambdaD Para.lambdaA Para.lambdaS Para.LatentDim Para.sigmaX AccLat]];
                        fprintf('trial=%d lambdaD=%g lambdaA=%g lambdaS=%g Lat=%d sigma=%g\n latent acc=%.2f\n\n',...
                            trial,Para.lambdaD,Para.lambdaA,Para.lambdaS,Para.LatentDim,Para.sigmaX,100*AccLat);
    ...
    [PerfLatent.bestAccTrial,indx] = max(Perf_Lat(:,6));
    ...
    PerfLatent.bestSigma = Perf_Lat(indx,5);
    ...
    bestAccTrial_Dim(trial) = PerfLatent.bestLatentDim;
    bestAccTrial_Sigma(trial) = PerfLatent.bestSigma;
