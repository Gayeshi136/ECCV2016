...
Para.lambdaA_range = [1e-3 1e-4 1e-5];
Para.lambdaS_range = [1e-3 1e-4 1e-5];
Para.LatentDim_range = [50 70 90 ];
Para.sigmaY_range = [0.15 0.2 0.25 0.3 0.4];
...
model_path = '/import/geb-experiments-archive/Alex/Joint/HMDB51/VideoStory/KLIEPX/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/Joint/HMDB51/VideoStory/KLIEPX/Perf/';
...
for trial = 1
    ...
    for sigmaY = Para.sigmaY_range
        ...
        for LatentDim = Para.LatentDim_range
            ...
            perf_filepath = fullfile(perf_path,sprintf('Perf_aug-%d_trial-%d_D-%g_A-%g_S-%g_lat-%d_X-%g_weight.mat',...
                Para.DataAug,trial,Para.lambdaD,Para.lambdaS,Para.lambdaA,Para.LatentDim,Para.sigmaY));
            ...
            if ~exist(perf_filepath,'file')
                ...
                if ~corrupted
                    ...
                    Perf_Lat = [Perf_Lat;[Para.sigmaY Para.LatentDim AccLat]];
                    fprintf('trial=%d sigmaY=%g Lat=%d\n latent acc=%.2f\n\n',...
                        trial,Para.sigmaY,Para.LatentDim,100*AccLat);
            ...
            model_filepath = sprintf([model_path,...
                'VideoStory_trial-%d_lambdaD-%g_lambdaA-%g_lambdaS-%g_LatDim-%d_sigY-%g_aug-1_KLEIPX_ccv_weight.mat'],...
            ...
            Perf_Lat = [Perf_Lat;[Para.sigmaY Para.LatentDim AccLat]];
            fprintf('trial=%d lsigmaY=%g Lat=%d\n latent acc=%.2f\n\n',...
                trial,Para.sigmaY,Para.LatentDim,100*AccLat);
...
PerfLatent.bestsigmaY = Perf_Lat(indx,1);
...
bestAccTrial_sigmaY(trial) = PerfLatent.bestsigmaY;
