...
for trial = 1:5
        ...
        for lambdaA = lambdaA_range
            for lambdaS = lambdaS_range
                for LatentDim = LatentDim_range
                    ...
                    [map,ap] = func_ts_ZSL_VideoStory_MAP_zscore_Distributed(Para,Model);
                    
                    Perf_Dist = [Perf_Dist;[Para.lambdaD Para.lambdaA Para.lambdaS Para.LatentDim map]];
                    fprintf('Distributed lambdaD=%g lambdaA=%g lambdaS=%g latent=%d acc=%.2f\n',...
                        Para.lambdaD,Para.lambdaA,Para.lambdaS,Para.LatentDim,100*map);
                    
                    [map,ap] = func_ts_ZSL_VideoStory_MAP_zscore_Latent(Para,Model);
                    Perf_Latent = [Perf_Latent;[Para.lambdaD Para.lambdaA Para.lambdaS Para.LatentDim map]];
                    fprintf('Latent lambdaD=%g lambdaA=%g lambdaS=%g latent=%d acc=%.2f\n',...
                        Para.lambdaD,Para.lambdaA,Para.lambdaS,Para.LatentDim,100*map);
                end
            end
        end
    ...
    [PerfDistributed.bestAccTrial(trial),indx] = max(Perf_Dist(:,5));
    PerfDistributed.bestLambdaD(trial) = Perf_Dist(indx,1);
    PerfDistributed.bestLambdaA(trial) = Perf_Dist(indx,2);
    PerfDistributed.bestLambdaS(trial) = Perf_Dist(indx,3);
    PerfDistributed.bestLatentDim(trial) = Perf_Dist(indx,4);
    
    [PerfLatent.bestAccTrial(trial),indx] = max(Perf_Latent(:,5));
    PerfLatent.bestLambdaD(trial) = Perf_Latent(indx,1);
    PerfLatent.bestLambdaA(trial) = Perf_Latent(indx,2);
    PerfLatent.bestLambdaS(trial) = Perf_Latent(indx,3);
    PerfLatent.bestLatentDim(trial) = Perf_Latent(indx,4);
end
