...
for trial = 1:5
    ...
    for lambda0 = lambda0_range
        for lambdat = lambdat_range
            ...
            %     [map,ap] = func_ts_ZSL_RMTL_MAP_Dim(Para,Model);
            %
            %     TrialMap(trial) = map;
            %
            [map,ap] = func_ts_ZSL_RMTL_MAP_Dim(Para,Model);
            
            Perf_Dist = [Perf_Dist;[Para.lambda0 Para.lambdat map]];
            fprintf('lambda0=%g lambdat=%g map=%.2f\n',...
                Para.lambda0,Para.lambdat,100*map);
            
            [map,ap] = func_ts_ZSL_RMTL_MAP_Dim_Latent(Para,Model);
            
            Perf_Lat = [Perf_Lat;[Para.lambda0 Para.lambdat map]];
            fprintf('lambda0=%g lambdat=%g map=%.2f\n',...
                Para.lambda0,Para.lambdat,100*map);
    ...
    [PerfDistributed.bestMapTrial(trial),indx] = max(Perf_Dist(:,3));
    PerfDistributed.bestLambda0(trial) = Perf_Dist(indx,1);
    PerfDistributed.bestLambdat(trial) = Perf_Dist(indx,2);
    
    [PerfLatent.bestMapTrial(trial),indx] = max(Perf_Lat(:,3));
    PerfLatent.bestLambdaD(trial) = Perf_Lat(indx,1);
    PerfLatent.bestLambdaA(trial) = Perf_Lat(indx,2);
    
end

mean(PerfDistributed.bestMapTrial)
std(PerfDistributed.bestMapTrial)

mean(PerfLatent.bestMapTrial)
std(PerfLatent.bestMapTrial)
