...
%                 continue;
            ...
            load(fullfile(nonoverlap_path,'USAA_Overlap.mat'),'exl_idx_HMDB51','exl_idx_OlympicSports','exl_idx_UCF101');
            ...
            aug_tr_idx = [idx_HMDB51(exl_idx_HMDB51) idx_UCF101(exl_idx_UCF101) idx_OlympicSports(exl_idx_OlympicSports) selected_tr_idx];
            
            Z = [tr_LabelVec_HMDB51(exl_idx_HMDB51,:) ; tr_LabelVec_UCF101(exl_idx_UCF101,:) ;...
                tr_LabelVec_OlympicSports(exl_idx_OlympicSports,:) ; tr_LabelVec];
            
            [~,ia,ic ] = unique(Z,'rows','stable');
            V_aug = func_L2Normalization( Z(ia,:))';
            
            K = D_CCV(aug_tr_idx,aug_tr_idx);
            Y = zeros(size(V_aug,2),size(K,1));
            for i = 1:length(ic)
                Y(ic(i),i) = 1;
            end
            ...
            Model.A=func_KernelizedL2Classification(K,V_aug,Y,Para);
