...
model_path = '/import/geb-experiments-archive/Alex/MTL/USAA/VideoStory/Model/';
if ~exist(model_path,'dir')
    mkdir(model_path);
end

perf_path = '/import/geb-experiments-archive/Alex/MTL/USAA/VideoStory/Perf/';
        ...
        for lambdaA = lambdaA_range
            for lambdaS = lambdaS_range
                for LatentDim = Latent_range
                    ...
                    %% Embedding Testing Data and Prototypes
                    D_map = Model.D;
                    A = Model.A;
                    S = Model.S;
                    L = Model.L;
                    ...
                    tr_LabelVec_USAA = phrasevec_mat(Para.ClassNoPerVideo,:);
                    tr_label_idx = [idx_HMDB51  idx_UCF101 idx_OlympicSports idx_OlympicSports(end)+find(tr_sample_ind==1)'];
                    [Z_All,mu,sig] = zscore(func_L2Normalization([tr_LabelVec_HMDB51 ; tr_LabelVec_UCF101 ; tr_LabelVec_OlympicSports; tr_LabelVec_USAA]));
                    Prototype = (func_L2Normalization(Para.phrasevec_mat)-repmat(mu,size(Para.phrasevec_mat,1),1))./repmat(sig,size(Para.phrasevec_mat,1),1);
                    Para.Prototype = Prototype(Para.idx_TestingSet,:);
                    
                    %% Test GOMTL Model
                    S_ts = D_map*A*D_CCV(aug_tr_idx,selected_ts_idx);   %D(selected_ts_idx,aug_tr_idx) * A;
                    Para.S_ts = zscore(S_ts');
                    
                    [meanAcc,Accuracy] = func_ts_ZSL_VideoStory_ACC_USAA_zscore_Distributed(Para,Model);
                    
                    Perf_Dist = [Perf_Dist;[lambdaS lambdaA LatentDim meanAcc]];
                    fprintf('lambdaS=%g lambdaA=%g latent=%d map=%.2f\n',...
                        lambdaS,lambdaA,LatentDim,100*meanAcc);
                    
                    
                    S_ts_Lat = A*D_CCV(aug_tr_idx,selected_ts_idx);
                    Para.S_ts_Lat = zscore(S_ts_Lat');
                    Para.Prototype_Lat = (Prototype(Para.idx_TestingSet,:)*pinv(D_map)');
                    
                    [meanAcc,Accuracy] = func_ts_ZSL_VideoStory_ACC_USAA_zscore_Latent(Para,Model);
                    
                    Perf_Lat = [Perf_Lat;[lambdaS lambdaA LatentDim meanAcc]];
                    fprintf('lambdaD=%g lambdaS=%g lambdaA=%g latent=%d map=%.2f\n',...
                        lambdaD,lambdaS,lambdaA,LatentDim,100*meanAcc);
